...
fn emit_project_config_warnings(app_event_tx: &AppEventSender, config: &Config) {
    ...
    let mut message = concat!(
        "以下目录中的项目 config.toml 已禁用。 ",
        "这些文件里的设置会被忽略，但技能和执行策略仍会加载。\n",
    )
    .to_string();
...
impl App {
    ...
    fn open_url_in_browser(&mut self, url: String) {
        if let Err(err) = webbrowser::open(&url) {
            self.chat_widget
                .add_error_message(format!("打开浏览器失败（{url}）：{err}"));
            return;
        }
    ...
    fn thread_label(&self, thread_id: ThreadId) -> String {
        ...
        if let Some(entry) = self.agent_picker_threads.get(&thread_id) {
            ...
            if label == format_agent_picker_item_name(None, None, false) {
                let thread_id = thread_id.to_string();
                let short_id: String = thread_id.chars().take(8).collect();
                format!("智能体 ({short_id})")
            } else {
    ...
    async fn submit_op_to_thread(&mut self, thread_id: ThreadId, op: Op) {
        ...
        let submitted = if self.active_thread_id == Some(thread_id) {
            ...
            match self.server.get_thread(thread_id).await {
                ...
                Err(err) => {
                    self.chat_widget
                        .add_error_message(format!("为审批响应查找线程 {thread_id} 失败：{err}"));
                    false
                }
            }
        };
    ...
    async fn open_agent_picker(&mut self) {
        ...
        if self.agent_picker_threads.is_empty() {
            self.chat_widget
                .add_info_message("暂无可用智能体。".to_string(), None);
            return;
        }
        ...
        self.chat_widget.show_selection_view(SelectionViewParams {
            title: Some("多智能体".to_string()),
            subtitle: Some("选择要关注的智能体线程".to_string()),
    ...
    async fn select_agent_thread(&mut self, tui: &mut tui::Tui, thread_id: ThreadId) -> Result<()> {
        ...
        let live_thread = match self.server.get_thread(thread_id).await {
            ...
            Err(err) => {
                if self.thread_event_channels.contains_key(&thread_id) {
                    ...
                    self.chat_widget
                        .add_error_message(format!("连接智能体线程 {thread_id} 失败：{err}"));
                    return Ok(());
                }
            }
        };
        ...
        if is_replay_only {
            self.chat_widget.add_info_message(
                format!("智能体线程 {thread_id} 已关闭，正在回放已保存的转录。"),
                None,
            );
        }
        ...
        match event {
            ...
            AppEvent::ForkCurrentSession => {
                ...
                if let Some(path) = self.chat_widget.rollout_path() {
                    ...
                    if path.exists() {
                        ...
                        self.chat_widget
                            .add_error_message("线程至少包含一轮对话后才能分叉。".to_string());
                    }
                } else {
                    self.chat_widget
                        .add_error_message("线程至少包含一轮对话后才能分叉。".to_string());
            ...
            AppEvent::PersistPersonalitySelection { personality } => {
                    ...
                    .await
                {
                    Ok(()) => {
                        ...
                        let mut message = format!("人格风格已设为 {label}");
                        if let Some(profile) = profile {
                            message.push_str("（");
                            message.push_str(profile);
                            message.push_str(" 配置）");
                    ...
                    Err(err) => {
                        ...
                        if let Some(profile) = profile {
                            self.chat_widget.add_error_message(format!(
                                "保存配置 `{profile}` 的人格风格失败：{err}"
                            ));
                        } else {
                            self.chat_widget
                                .add_error_message(format!("保存默认人格风格失败：{err}"));
                        }
                    }
                }
            }
                ...
                match builder.apply().await {
                    Ok(()) => {
                        ...
                        if self.chat_widget.realtime_conversation_is_live() {
                            ...
                            self.chat_widget.add_info_message(
                                format!("实时{}设备已设为 {selection}", kind.noun()),
                                None,
                            );
                        }
                    }
                    Err(err) => {
                        ...
                        self.chat_widget
                            .add_error_message(format!("保存实时{}设备失败：{err}", kind.noun()));
                    }
                }
            }
            ...
            AppEvent::UpdateFeatureFlags { updates } => {
                ...
                if let Err(err) = builder.apply().await {
                    ...
                    self.chat_widget
                        .add_error_message(format!("更新实验功能失败：{err}"));
                }
            }
            ...
            AppEvent::PersistRateLimitSwitchPromptHidden => {
                    ...
                    .await
                {
                    ...
                    self.chat_widget
                        .add_error_message(format!("保存限额提醒偏好失败：{err}"));
                }
            }
            AppEvent::PersistPlanModeReasoningEffort(effort) => {
                    ...
                    .await
                {
                    ...
                    if let Some(profile) = profile {
                        self.chat_widget.add_error_message(format!(
                            "保存配置 `{profile}` 的计划模式推理级别失败：{err}"
                        ));
                    } else {
                        self.chat_widget
                            .add_error_message(format!("保存计划模式推理级别失败：{err}"));
                    }
                }
            }
            ...
            AppEvent::StatusLineSetup { items } => {
                ...
                match apply_result {
                    ...
                    Err(err) => {
                        ...
                        self.chat_widget
                            .add_error_message(format!("保存状态栏项失败：{err}"));
                    }
                }
            }
