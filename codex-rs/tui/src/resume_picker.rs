...
impl SessionPickerAction {
    fn title(self) -> &'static str {
        match self {
            SessionPickerAction::Resume => "恢复历史会话",
            SessionPickerAction::Fork => "分叉历史会话",
        }
    }
...
fn render_list(
    ...
    if state.pagination.loading.is_pending() && y < area.y.saturating_add(area.height) {
        let loading_line: Line = vec!["  ".into(), "正在加载更早会话…".italic().dim()].into();
        let rect = Rect::new(area.x, y, area.width, 1);
...
fn render_empty_state_line(state: &PickerState) -> Line<'static> {
    ...
    if state.pagination.loading.is_pending() {
        return vec!["正在加载更早会话…".italic().dim()].into();
    }
...
mod tests {
    ...
    //         );
    //
    //         frame.render_widget_ref(Line::from("输入关键字搜索".dim()), search);
    //
