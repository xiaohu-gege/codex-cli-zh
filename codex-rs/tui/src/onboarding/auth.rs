...
impl AuthModeWidget {
    ...
    fn render_pick_mode(&self, area: Rect, buf: &mut Buffer) {
        let mut lines: Vec<Line> = vec![
            Line::from(vec![
                "  ".into(),
                "使用 ChatGPT 登录，在你的付费套餐内使用 Codex".into(),
            ]),
            Line::from(vec!["  ".into(), "或连接 API Key 按量计费".into()]),
            "".into(),
        ];
        ...
        if !self.is_api_login_allowed() {
            lines.push(
                "  此工作区已禁用 API Key 登录，请使用 ChatGPT 登录后继续。"
                    .dim()
                    .into(),
            );
            lines.push("".into());
    ...
    fn render_continue_in_browser(&self, area: Rect, buf: &mut Buffer) {
        ...
        {
            lines.push("  如果链接未自动打开，请用下方链接完成认证：".into());
            lines.push("".into());
    ...
    fn render_api_key_configured(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            "✓ 已配置 API Key".fg(Color::Green).into(),
            "".into(),
            "  Codex 将使用你的 API Key 按量计费。".into(),
        ];
    ...
    fn render_api_key_entry(&self, area: Rect, buf: &mut Buffer, state: &ApiKeyInputState) {
        ...
        if state.prepopulated_from_env {
            intro_lines.push("  已检测到 OPENAI_API_KEY 环境变量。".into());
            intro_lines.push("  若希望使用其他账号，可粘贴不同的 Key。".dim().into());
