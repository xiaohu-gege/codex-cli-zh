...
const PLAN_IMPLEMENTATION_TITLE: &str = "实现这个计划吗？";
        ...
        if let Some(secondary_used_percent) = secondary_used_percent {
            ...
            if let Some(threshold) = highest_secondary {
                ...
                warnings.push(format!(
                    "提醒：你的 {limit_label} 配额剩余不足 {remaining_percent:.0}%。可运行 /status 查看详情。"
                ));
            }
        }
...
impl ChatWidget {
    ...
    fn log_websocket_timing_totals(&mut self, delta: RuntimeMetricsSummary) {
        if let Some(label) = history_cell::runtime_metrics_label(delta.responses_api_summary()) {
            self.add_plain_history_lines(vec![
                vec!["• ".dim(), format!("WebSocket 时延：{label}").dark_gray()].into(),
            ]);
        }
    }
    ...
    fn open_plan_implementation_prompt(&mut self) {
        ...
        let items = vec![
            ...
            SelectionItem {
                ...
                description: Some("继续让模型进行规划。".to_string()),
                selected_description: None,
                is_current: false,
                actions: Vec::new(),
                dismiss_on_select: true,
        ...
        if let Some(current) = &self.mcp_startup_status {
            ...
            if let Some(first) = starting.first() {
                ...
                let header = if total > 1 {
                    format!(
                        "正在启动 MCP 服务（{completed}/{total}）：{}",
                        to_show.join(", ")
                    )
                } else {
                    format!("正在启动 MCP 服务：{first}")
                };
                self.set_status_header(header);
            }
        }
    ...
    fn on_interrupted_turn(&mut self, reason: TurnAbortReason) {
        ...
        if reason != TurnAbortReason::ReviewEnded {
            self.add_to_history(history_cell::new_error_event(
                "对话已中断，请告诉模型如何调整。若遇到问题，可用 `/feedback` 反馈。".to_owned(),
            ));
        }
    ...
    pub(crate) fn can_run_ctrl_l_clear_now(&mut self) -> bool {
        ...
        let message = "任务进行中，Ctrl+L 暂不可用。".to_string();
        self.add_to_history(history_cell::new_error_event(message));
        self.request_redraw();
        false
    }
        ...
        match cmd {
            ...
            SlashCommand::Init => {
                ...
                if init_target.exists() {
                    let message = format!(
                        "当前目录已存在 {DEFAULT_PROJECT_DOC_FILENAME}。为避免覆盖，已跳过 /init。"
                    );
                    self.add_info_message(message, None);
                    return;
                }
            ...
            SlashCommand::Plan => {
                ...
                if let Some(mask) = collaboration_modes::plan_mask(self.models_manager.as_ref()) {
                    ...
                    self.add_info_message("当前计划模式不可用。".to_string(), None);
                }
            }
            SlashCommand::Collab => {
                if !self.collaboration_modes_enabled() {
            ...
            SlashCommand::Copy => {
                let Some(text) = self.last_copyable_output.as_deref() else {
                    self.add_info_message(
                        "在 Codex 首次输出前，或刚回滚后，`/copy` 暂不可用。".to_string(),
                        None,
                    );
                    return;
                };
                ...
                match copy_result {
                    Ok(()) => {
                        let hint = self.agent_turn_running.then_some(
                            "当前回合仍在进行，已复制最近一次完成的输出（不包含生成中的响应）。"
                                .to_string(),
                        );
                        self.add_info_message(
            ...
            SlashCommand::Rollout => {
                if let Some(path) = self.rollout_path() {
                    self.add_info_message(format!("当前会话记录路径：{}", path.display()), None);
                } else {
                    self.add_info_message("会话记录路径暂不可用。".to_string(), None);
                }
            }
        ...
        match cmd {
            SlashCommand::Rename if !trimmed.is_empty() => {
                ...
                let Some(name) = codex_core::util::normalize_thread_name(&prepared_args) else {
                    self.add_error_message("线程名不能为空。".to_string());
                    return;
                };
            ...
            Box::new(move |name: String| {
                let Some(name) = codex_core::util::normalize_thread_name(&name) else {
                    tx.send(AppEvent::InsertHistoryCell(Box::new(
                        history_cell::new_error_event("线程名不能为空。".to_string()),
                    )));
                    return;
                };
    ...
    fn dispatch_event_msg(
        ...
        match msg {
            ...
            EventMsg::TurnAborted(ev) => match ev.reason {
                ...
                TurnAbortReason::Replaced => self.on_error("回合已中止：被新任务替换".to_owned()),
    ...
    fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
        let description = Self::model_preset_menu_description(&preset)
            .or_else(|| Some("后续回合将消耗更少额度。".to_string()));
        ...
        let items = vec![
            ...
            SelectionItem {
                ...
                description: Some("继续使用当前模型，不进行切换。".to_string()),
                selected_description: None,
                is_current: false,
            ...
            SelectionItem {
                ...
                description: Some("后续不再提示切换模型的限额提醒。".to_string()),
                selected_description: None,
                is_current: false,
    ...
    pub(crate) fn open_model_popup(&mut self) {
        if !self.is_session_configured() {
            self.add_info_message("启动完成前无法选择模型。".to_string(), None);
            return;
        }
        ...
        let presets: Vec<ModelPreset> = match self.models_manager.try_list_models() {
            Ok(models) => models,
            Err(_) => {
                self.add_info_message("模型列表更新中，请稍后再试 /model。".to_string(), None);
                return;
            }
        };
    ...
    pub(crate) fn open_personality_popup(&mut self) {
        if !self.is_session_configured() {
            self.add_info_message("启动完成前无法选择人格风格。".to_string(), None);
            return;
        }
        if !self.current_model_supports_personality() {
            let current_model = self.current_model();
            self.add_error_message(format!(
                "当前模型（{current_model}）不支持人格风格。请用 /model 切换到其他模型。"
            ));
            return;
        }
    ...
    fn open_personality_popup_for_current_model(&mut self) {
        ...
        header.push(Line::from("选择人格风格".bold()));
    ...
    pub(crate) fn open_realtime_audio_device_selection(&mut self, kind: RealtimeAudioDeviceKind) {
        match list_realtime_audio_device_names(kind) {
            ...
            Err(err) => {
                self.add_error_message(format!("加载实时语音{}设备失败：{err}", kind.noun()));
            }
        }
    }
    ...
    fn open_realtime_audio_device_selection_with_names(
        ...
        header.push(Line::from("已保存设备仅应用于实时语音。".dim()));
    ...
    pub(crate) fn open_realtime_audio_restart_prompt(&mut self, kind: RealtimeAudioDeviceKind) {
        ...
        let items = vec![
            ...
            SelectionItem {
                name: "稍后应用".to_string(),
                description: Some(format!("保持当前{}，直到下次启动本地音频。", kind.noun())),
                dismiss_on_select: true,
                ..Default::default()
            },
        ];
    ...
    fn model_menu_warning_line(&self) -> Option<Line<'static>> {
        ...
        let warning = format!(
            "警告：OPENAI_BASE_URL 已设置为 {base_url}。模型选择可能不受支持或无法正常工作。"
        );
    ...
    fn model_preset_menu_description(preset: &ModelPreset) -> Option<String> {
        let trimmed_description = preset.description.trim();
        ...
        let mut efforts: Vec<&str> = preset
            .supported_reasoning_efforts
            .iter()
            .map(|option| Self::reasoning_effort_label(option.effort))
            .collect();
        ...
        let supported = efforts.join("、");
        let default = Self::reasoning_effort_label(preset.default_reasoning_effort);
        Some(format!("支持推理级别：{supported}；默认：{default}"))
    }
    ...
    pub(crate) fn open_model_popup_with_presets(&mut self, presets: Vec<ModelPreset>) {
        ...
        if !other_presets.is_empty() {
            ...
            let description = Some(format!("选择具体模型与推理级别（当前：{current_label}）"));
        ...
        let header = self.model_menu_header("选择模型", "可选择快捷自动模式，或浏览全部模型。");
        self.bottom_pane.show_selection_view(SelectionViewParams {
            footer_hint: Some(standard_popup_hint_line()),
            items,
            header,
    ...
    pub(crate) fn open_all_models_popup(&mut self, presets: Vec<ModelPreset>) {
        if presets.is_empty() {
            self.add_info_message("当前没有更多可用模型。".to_string(), None);
            return;
        }
        ...
        let header = self.model_menu_header(
            "选择模型与推理级别",
            "可通过 codex -m <model_name> 或 config.toml 使用旧版模型",
        );
        self.bottom_pane.show_selection_view(SelectionViewParams {
            footer_hint: Some("按 Enter 选择推理级别，按 Esc 关闭。".into()),
            items,
            header,
            ..Default::default()
        });
    ...
    pub(crate) fn open_collaboration_modes_popup(&mut self) {
        ...
        let items: Vec<SelectionItem> = presets
            ...
            .map(|mask| {
                let name = mask
                    .mode
                    .map(Self::collaboration_mode_display_name)
                    .map(str::to_string)
                    .unwrap_or_else(|| mask.name.clone());
                let description = match mask.mode {
                    Some(ModeKind::Default) => Some("直接进入执行流程，适合快速开发。".to_string()),
                    Some(ModeKind::Plan) => Some("优先规划再执行，适合复杂任务。".to_string()),
                    Some(ModeKind::PairProgramming) => Some("结对协作式流程。".to_string()),
                    Some(ModeKind::Execute) => Some("偏执行导向流程。".to_string()),
                    None => None,
                };
    ...
    pub(crate) fn open_plan_reasoning_scope_prompt(
        ...
        let all_modes_description =
            format!("设置全局默认推理级别并更新计划模式覆盖。将替换当前{plan_reasoning_source}。");
    ...
    pub(crate) fn open_reasoning_popup(&mut self, preset: ModelPreset) {
        ...
        let warning_text = warn_effort.map(|effort| {
            ...
            format!("⚠ {effort_label} 推理强度会较快消耗 Plus 计划限额。")
        });
        ...
        for choice in choices.iter() {
            ...
            let description = choice
                ...
                .map(|effort| match effort {
                    ReasoningEffortConfig::None => "关闭额外推理，响应速度最快。",
                    ReasoningEffortConfig::Minimal => "使用极低推理强度，适合快速确认。",
                    ReasoningEffortConfig::Low => "使用低推理强度，适合常规简单任务。",
                    ReasoningEffortConfig::Medium => "使用中等推理强度，兼顾质量与速度。",
                    ReasoningEffortConfig::High => "使用高推理强度，适合复杂分析与规划。",
                    ReasoningEffortConfig::XHigh => "使用超高推理强度，适合高复杂度问题。",
                })
                .map(str::to_string);
    ...
    pub(crate) fn open_permissions_popup(&mut self) {
        ...
        let footer_note = show_elevate_sandbox_hint.then(|| {
            vec![
                "非管理员沙箱在多数情况下可保护文件并限制网络访问，但在提示注入场景下风险更高。若要升级到默认沙箱，请运行 ".dim(),
    ...
    pub(crate) fn open_full_access_confirmation(
        ...
        let info_line = Line::from(vec![
            "当 Codex 以完全访问权限运行时，它可以在未经你确认的情况下编辑你电脑上的任意文件，并执行可联网命令。"
                .into(),
            " 启用完全访问权限请务必谨慎，这会显著增加数据丢失、泄露或异常行为的风险。"
                .fg(Color::Red),
        ]);
        ...
        let items = vec![
            SelectionItem {
                name: "是，继续".to_string(),
                description: Some("为本会话启用完全访问权限".to_string()),
            ...
            SelectionItem {
                name: "取消".to_string(),
                description: Some("返回，不启用完全访问权限".to_string()),
    ...
    pub(crate) fn open_world_writable_warning_confirmation(
        ...
        let info_line = if failed_scan {
            Line::from(vec![
                "无法完成“所有人可写”扫描，因此无法验证保护是否生效。".into(),
                format!(" 在 {mode_label} 下，Windows 沙箱无法保证防护效果。").fg(Color::Red),
            ])
        } else {
            Line::from(vec![
                "Windows 沙箱无法保护对“所有用户可写（Everyone）”目录的写入。".into(),
                " 建议移除以下目录的“Everyone（所有用户）”写权限：".into(),
            ])
        };
        header_children.push(Box::new(
            Paragraph::new(vec![info_line]).wrap(Wrap { trim: false }),
        ...
        let items = vec![
            SelectionItem {
                name: "继续".to_string(),
                description: Some(format!("本会话应用 {mode_label}")),
                actions: accept_actions,
                dismiss_on_select: true,
                ..Default::default()
            },
            SelectionItem {
                name: "继续且不再提醒".to_string(),
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(&mut self, preset: ApprovalPreset) {
        ...
        if !codex_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED {
            ...
            header.push(*Box::new(
                Paragraph::new(vec![
                    line![
                        "Windows 上的智能体模式会使用实验性沙箱来限制网络和文件系统访问。".bold()
                    ],
            ...
            let items = vec![
                SelectionItem {
                    ...
                    description: Some("为当前会话启用非管理员沙箱保护。".to_string()),
                    actions: vec![Box::new(move |tx| {
                        tx.send(AppEvent::EnableWindowsSandboxForAgentMode {
                            preset: preset_clone.clone(),
                ...
                SelectionItem {
                    name: "返回".to_string(),
                    description: Some("返回权限菜单，不修改当前设置。".to_string()),
        ...
        header.push(*Box::new(
            Paragraph::new(vec![
                line!["设置 Codex 智能体沙箱，以保护文件并控制网络访问。了解更多 <https://developers.openai.com/codex/windows>"],
            ])
            .wrap(Wrap { trim: false }),
        ));
        ...
        let items = vec![
            SelectionItem {
                ...
                description: Some("通过管理员权限完成默认沙箱安装与配置。".to_string()),
                actions: vec![Box::new(move |tx| {
            ...
            SelectionItem {
                name: "使用非管理员沙箱（提示注入风险更高）".to_string(),
                description: Some("无需管理员权限继续，但隔离能力较弱。".to_string()),
                actions: vec![Box::new(move |tx| {
            ...
            SelectionItem {
                name: "退出".to_string(),
                description: Some("退出 Codex，不继续当前设置流程。".to_string()),
                actions: vec![Box::new(move |tx| {
    ...
    pub(crate) fn open_windows_sandbox_fallback_prompt(&mut self, preset: ApprovalPreset) {
        ...
        let items = vec![
            SelectionItem {
                name: "重试管理员沙箱设置".to_string(),
                description: Some("再次尝试以管理员权限完成默认沙箱配置。".to_string()),
                actions: vec![Box::new({
            ...
            SelectionItem {
                name: "使用非管理员沙箱继续".to_string(),
                description: Some("跳过管理员配置，使用兼容模式继续。".to_string()),
                actions: vec![Box::new({
                    let otel = self.otel_manager.clone();
    ...
    fn collaboration_mode_label(&self) -> Option<&'static str> {
        ...
        active_mode
            .is_tui_visible()
            .then_some(Self::collaboration_mode_display_name(active_mode))
    }

    fn collaboration_mode_display_name(mode: ModeKind) -> &'static str {
        match mode {
            ModeKind::Plan => "计划",
            ModeKind::Default => "默认",
    ...
    fn personality_description(personality: Personality) -> &'static str {
        match personality {
            Personality::None => "不附加额外人格指令。",
            Personality::Friendly => "语气温暖、协作友好、乐于帮助。",
            Personality::Pragmatic => "表达简洁、任务导向、直接明确。",
        }
    }
    ...
    pub(crate) fn set_collaboration_mask(&mut self, mut mask: CollaborationModeMask) {
            ...
            && (previous_model != next_model || previous_effort != next_effort)
        {
            ...
            message.push_str("（");
            message.push_str(Self::collaboration_mode_display_name(next_mode));
            message.push_str("模式）");
            self.add_info_message(message, None);
        }
        self.request_redraw();
    ...
    pub(crate) fn add_connectors_output(&mut self) {
        if !self.connectors_enabled() {
            self.add_info_message(
                "应用功能已禁用。".to_string(),
                Some("启用 apps 功能后可使用 $ 或 /apps。".to_string()),
            );
            return;
        }
        ...
        match connectors_cache {
            ConnectorsCacheState::Ready(snapshot) => {
                if snapshot.connectors.is_empty() {
                    self.add_info_message("暂无可用应用。".to_string(), None);
                } else {
    ...
    fn connectors_popup_params(&self, connectors: &[connectors::AppInfo]) -> SelectionViewParams {
        ...
        for connector in connectors {
            ...
            let selected_label = if is_installed {
                format!("{status_label}。按 Enter 打开应用页面以安装、管理或启用/禁用该应用。")
            } else {
                format!("{status_label}。按 Enter 打开应用页面安装该应用。")
            };
            let missing_label = format!("{status_label}。应用链接不可用。");
            let instructions = if connector.is_accessible {
                "请在浏览器中管理该应用。"
            } else {
                "请在浏览器中安装该应用，然后重新加载 Codex。"
            };
        ...
        SelectionViewParams {
            ...
            items,
            is_searchable: true,
            search_placeholder: Some("输入关键字搜索应用".to_string()),
    ...
    pub(crate) fn submit_user_message_with_mode(
        ...
        {
            self.add_error_message("回合进行中，无法切换协作模式。".to_string());
            return;
        }
    ...
    pub(crate) fn open_review_popup(&mut self) {
        ...
        items.push(SelectionItem {
            ...
            description: Some("比较当前分支与目标基线分支（PR 风格）。".into()),
            actions: vec![Box::new({
                let cwd = self.config.cwd.clone();
                move |tx| {
        ...
        items.push(SelectionItem {
            ...
            description: Some("直接审查当前工作区中尚未提交的改动。".to_string()),
        ...
        items.push(SelectionItem {
            name: "评审指定提交".to_string(),
            description: Some("从最近提交中选择一个提交进行评审。".to_string()),
            actions: vec![Box::new({
                let cwd = self.config.cwd.clone();
                move |tx| {
        ...
        items.push(SelectionItem {
            name: "自定义评审指令".to_string(),
            description: Some("输入自定义评审要求并发起评审。".to_string()),
            actions: vec![Box::new(move |tx| {
    ...
    pub(crate) async fn show_review_branch_picker(&mut self, cwd: &Path) {
        ...
        let current_branch = current_branch_name(cwd)
            .await
            .unwrap_or_else(|| "(分离 HEAD)".to_string());
        ...
        for option in branches {
            ...
            items.push(SelectionItem {
                ...
                description: Some("将当前分支与该基线分支进行对比评审。".to_string()),
    ...
    pub(crate) async fn show_review_commit_picker(&mut self, cwd: &Path) {
        ...
        for entry in commits {
            ...
            items.push(SelectionItem {
                name: subject.clone(),
                description: Some(format!("提交哈希：{short_sha}")),
                actions: vec![Box::new(move |tx3: &AppEventSender| {
                    tx3.send(AppEvent::CodexOp(Op::Review {
                        review_request: ReviewRequest {
    ...
    fn display(&self) -> String {
        match self {
            ...
            Notification::ExecApprovalRequested { command } => {
                format!("需要审批：{}", truncate_text(command, 30))
            }
